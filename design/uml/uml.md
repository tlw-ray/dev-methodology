...
---
   ...
   2）扩展关系（extend）：是指一个用例对另外一个用例的功能扩展。注意在绘图时，带箭头的线要**从扩展用例指向被扩展的用例**。
...
多个子状态之间存在**顺序和并发**两种关系。如下图所示，1、2为顺序子状态（Sequence State），1、3为并发子状态（ConcurrentState）。
